...
def planning(goal):
    ...
    desired_position_.x = goal.target_x
    desired_position_.y = goal.target_y
    rospy.set_param('des_pos_x', desired_position_.x)
    rospy.set_param('des_pos_y', desired_position_.y)
    
    #feedback and result from PositionsAction
    feedback = assignment_2_2022.msg.PositionsFeedback()
    result = assignment_2_2022.msg.PositionsResult()
    ...
    while not rospy.is_shutdown():
        ...
        if act_s.is_preempt_requested():
            ...
            # Print log messages
            rospy.loginfo('Feedback, current x: {0}'.format(feedback.current_x))
            rospy.loginfo('Feedback, current y: {0}'.format(feedback.current_y))
            ...
            #change PositionsResult to True, meaning the target position was readched 
            result = assignment_2_2022.msg.PositionsResult()
...
def main():
    ...
    act_s = actionlib.SimpleActionServer('/reaching_goal',assignment_2_2022.msg.PositionsAction,planning, auto_start=False)
    act_s.start()
